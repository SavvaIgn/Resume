...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		BoardCell nextCell = this.getNextCellInDir();
		if (nextCell.isSpam() == true) {
			this.getSnakeHead().becomeBody();
			nextCell.becomeHead();
			this.snakeCells.add(nextCell);
		} else {
			BoardCell snakeTail = this.getSnakeTail();
			this.getSnakeTail().becomeOpen();
			this.getSnakeHead().becomeBody();
			nextCell.becomeHead();
			this.snakeCells.add(nextCell);
			this.snakeCells.remove(snakeTail);
		}
	}
	...
	public BoardCell getNextCellInDir() {
		...
		} else { // because we need to return something even when in AI mode, so west neighbor
					// works
			return this.getWestNeighbor();
		}
	}
	...
	public BoardCell getNextCellFromBFS() {
		...
		while(!cellsToSearch.isEmpty()) {
			BoardCell cell = cellsToSearch.remove();

			for (int i = 0; i<5; i++) {
				if (i==0) {											// North neighbor
					closestSpamCell = this.getCell(cell.getRow()+1, cell.getColumn()); 
				}
				if (i==1) {											//South Neighbor
					closestSpamCell = this.getCell(cell.getRow()-1, cell.getColumn()); //
				}
				if(i==2) {											//East Neighbor
					closestSpamCell = this.getCell(cell.getRow(), cell.getColumn()+1);
				}
				if(i==3) {											//West neighbor
					closestSpamCell = this.getCell(cell.getRow(), cell.getColumn()-1);
				}
				if(!closestSpamCell.inSearchListAlready() && closestSpamCell.isOpen()) { //
					cellsToSearch.add(closestSpamCell);
					closestSpamCell.setAddedToSearchList();
					closestSpamCell.setParent(cell);
					if(closestSpamCell.isSpam()) {
	...
	private BoardCell getFirstCellInPath(BoardCell start) { 
		if (start.getParent()==this.getSnakeHead()) {
			return start;
		} else {
			start = start.getParent();
			return this.getFirstCellInPath(start);
		}
	}
	...
	public void reverseSnake() {
		...
		tail.becomeHead();
		this.snakeCells.set(0, head);

		// Step 3: relabel the head
		this.snakeCells.set(this.snakeCells.size() - 1, tail);

		// Step 4: calculate the new direction after reversing!
		if (getNorthNeighbor(tail).isBody()) {
			this.setDirectionSouth();
		} else if (getSouthNeighbor(tail).isBody()) {
			this.setDirectionNorth();
		} else if (getEastNeighbor(tail).isBody()) {
			this.setDirectionWest();
		} else if (getWestNeighbor(tail).isBody()) {
			this.setDirectionEast();
		}
	}
